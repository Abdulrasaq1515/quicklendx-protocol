...
fn test_pagination_empty_whitelist_boundaries() {
    ...
    assert_eq!(
        result.len(),
        0,
        "empty whitelist with zero limit should return empty"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "empty whitelist with any limit should return empty"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "empty whitelist with max offset should return empty"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "empty whitelist with max limit should return empty"
    );
}
...
fn test_pagination_offset_saturation() {
    ...
    assert_eq!(
        result.len(),
        0,
        "offset at exact length should return empty"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "max offset should return empty without panic"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "near-max offset should return empty without panic"
    );
...
fn test_pagination_limit_saturation() {
    ...
    assert_eq!(
        result.len(),
        3,
        "limit larger than available should return all items"
    );
    ...
    assert_eq!(
        result.len(),
        3,
        "max limit should return all items without panic"
    );
    ...
    assert_eq!(
        result.len(),
        3,
        "limit matching count should return all items"
    );
    ...
    assert_eq!(
        result.len(),
        2,
        "limit less than count should return limited items"
    );
}
...
fn test_pagination_overflow_protection() {
    ...
    assert_eq!(
        result.len(),
        0,
        "max offset + max limit should return empty without panic"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "large offset with normal limit should return empty"
    );
    ...
    assert_eq!(
        result.len(),
        5,
        "normal offset with max limit should return remaining items"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "near-max offset with small limit should return empty"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "arithmetic overflow scenario should be handled safely"
    );
}
...
fn test_pagination_consistency_and_ordering() {
    ...
    for i in 0..3 {
        assert_eq!(
            page1.get(i).unwrap(),
            full_list.get(i).unwrap(),
            "page1 item {} should match full list",
            i
        );
    }
    for i in 0..3 {
        assert_eq!(
            page2.get(i).unwrap(),
            full_list.get(i + 3).unwrap(),
            "page2 item {} should match full list",
            i
        );
    }
    assert_eq!(
        page3.get(0).unwrap(),
        full_list.get(6).unwrap(),
    ...
    assert_eq!(
        overlap_page.len(),
        3,
        "overlapping page should have 3 items"
    );
    assert_eq!(
        overlap_page.get(0).unwrap(),
        full_list.get(2).unwrap(),
...
fn test_pagination_single_item_edge_cases() {
    ...
    assert_eq!(
        result.len(),
        1,
        "large limit should still return single item"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "offset beyond single item should return empty"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "zero limit should return empty even with item"
    );
}
...
fn test_pagination_after_modifications() {
    ...
    assert_eq!(
        page_before.len(),
        3,
        "should have 3 items before modification"
    );
    ...
    assert_eq!(
        full_list_after.len(),
        3,
        "should have 3 total items after removal"
    );
    assert!(
        !full_list_after.contains(&currencies[1]),
        "removed currency should not be present"
    );
    assert!(
        !full_list_after.contains(&currencies[3]),
        "removed currency should not be present"
    );
    ...
    assert_eq!(
        boundary_page.len(),
        0,
        "offset at new length should return empty"
    );
    ...
    assert_eq!(
        empty_page.len(),
        0,
        "pagination after clear should return empty"
    );
}
...
fn test_pagination_security_boundaries() {
    ...
    for i in 0..full_list.len() {
        assert_eq!(
            full_list.get(i).unwrap(),
            paginated_total.get(i).unwrap(),
            "item {} should match between full and paginated reads",
            i
        );
    }
}
...
fn test_pagination_large_dataset_boundaries() {
    ...
    let currencies: alloc::vec::Vec<Address> =
        (0..large_count).map(|_| Address::generate(&env)).collect();
    ...
    assert_eq!(
        result.len(),
        0,
        "large offset beyond dataset should return empty"
    );
    ...
    assert_eq!(
        result.len(),
        0,
        "offset at exact dataset size should return empty"
    );
}
...
fn test_pagination_concurrent_modification_boundaries() {
    ...
    assert_eq!(
        modified_page.len(),
        5,
        "page should still return 5 items after removal"
    );
...
fn test_pagination_address_handling_boundaries() {
    ...
    assert_eq!(
        result.len(),
        1,
        "duplicate adds should result in single entry"
    );

    // Test with many unique addresses
    let unique_currencies: alloc::vec::Vec<Address> =
    ...
    assert_eq!(
        total_count, 16u32,
        "should have 16 total currencies (1 + 15)"
    ); // 1 from duplicate test + 15 new
    ...
    assert_eq!(
        all_paginated.len(),
        16,
        "should retrieve all unique addresses"
    );
    ...
    for i in 0..all_paginated.len() {
        for j in (i + 1)..all_paginated.len() {
            assert_ne!(
                all_paginated.get(i).unwrap(),
                all_paginated.get(j).unwrap(),
...
fn test_pagination_storage_efficiency() {
    ...
    assert_eq!(
        empty_result.len(),
        0,
        "empty storage should return empty efficiently"
    );
    ...
    for (i, currency) in currencies.iter().enumerate() {
        ...
        assert_eq!(
            result.len(),
            count,
            "should return correct count at growth step {}",
            i
        );
        ...
        assert_eq!(
            boundary_result.len(),
            0,
            "boundary offset should return empty at step {}",
            i
        );
    }
    ...
    assert_eq!(
        cleared_result.len(),
        0,
        "cleared storage should return empty efficiently"
    );
